...
=======
import { View, Text, StyleSheet, TouchableOpacity, Dimensions, FlatList, Image, Modal, ScrollView, Alert } from "react-native";
import { SafeAreaView } from "react-native-safe-area-context";
import { useRoute } from '@react-navigation/native';
import DatePicker from 'react-native-modern-datepicker';
import Icons from 'react-native-vector-icons/dist/Ionicons';
...
const ScheduleAappointmnet = ({navigation}) => {
  const durations = [{id: '1', duration: 30}];
...
<<<<<<< Updated upstream
  ...
  const filterCategories = (type) => {
    if (!data || !data.speciality[0] || !data.speciality[0].subSpecializations)
      return;
=======
    const route = useRoute();
    const { data } = route.params;
    ...
    console.log('duration ', selectDur.duration)
    console.log('start date>>>>>>>>>>>>>>>>>>>', startDate);
    ...
    const currentDateTime = new Date();
    console.log('selectedCatC ', selectedCatC)
    const checkAndNavigate = async (appointmentDetails) => {

        try {
            const jsonValue = await AsyncStorage.getItem('schedule');
            if (jsonValue != null) {
                const data = JSON.parse(jsonValue);
                if (data.selectedTime === selectDur?.duration && data.date === startDate) {
    ...
    const saveDurationAndDate = async () => {
        try {
            const data = {
                selectedTime: selectDur?.duration,
                date: startDate,
            };
...
>>>>>>> Stashed changes
    ...
    const filteredCat = data.speciality[0].subSpecializations.filter(
      (sub) => sub.eligibility === eligibility
    );
    setCat(filteredCat);
  };
  ...
  const calculateEndDate = (start, duration) => {
    if (start && duration) {
      ...
      const formattedEndDate = moment(endMoment).format('YYYY-MM-DD')
  ...
  const renderTypeButton = (type) => (
    <TouchableOpacity onPress={() => handleTypeSelect(type)}>
      <View
        style={[styles.nameC, selectedType === type && styles.selectedButton]}
      >
        <Text
          style={
            selectedType === type
  ...
  function timeToTimestamp(timeStr) {
    ...
    const now = new Date();
    now.setHours(hours24, minutes, 0, 0); // Set hours, minutes, seconds, and milliseconds
    return Math.floor(now.getTime() / 1000); // Return timestamp in seconds
  }

  function dateToTimestampInSeconds(dateStr) {
  ...
  const handleBookAppointment = () => {
    ...
    if (
      selectDur.duration &&
      typeof selectDur.duration === 'string' &&
      selectDur.duration.includes(' - ')
    ) {
      ...
      const appointmentDetails = {
        doctor: data,
        startDate: dateToTimestampInSeconds(startDate),
        endDate: dateToTimestampInSeconds(endDate),
        type: selectedType,
        category: selectedCat,
        duration: selectDur.duration, // Full slot duration
        fromTime: timeToTimestamp(fromTime), // Extracted 'from' time
        toTime: timeToTimestamp(toTime), // Extracted 'to' time
      };
        ...
        <View style={styles.container}>
          <View style={styles.innerContainer}>
            ...
            <View style={styles.g1}>
              <Text style={styles.h12}>
                {data.firstName} {data.lastName}
              </Text>
              <Text style={styles.h2}>
                {data.speciality[0].specialization.name}
              </Text>
            </View>
...
<<<<<<< Updated upstream
            ...
            <View style={styles.wraprC}>
              <TouchableOpacity
                ...
                onPress={toggleDropdownC}
              >
                <Text style={{color: 'black'}}>
                  {selectedCatC?.specialization?.name || 'Category'}
                </Text>
                <Icons
                  name={isOpenC ? 'chevron-up' : 'chevron-down'}
                  size={24}
                  color="black"
                />
              </TouchableOpacity>
...
=======
    const handleBookAppointment = () => {
        if (!data || !startDate || !selectedCat || !selectDur.duration) {
            ...
            if (selectDur.duration && typeof selectDur.duration === 'string' && selectDur.duration.includes(' - ')) {
                ...
                checkAndNavigate(appointmentDetails)
...
>>>>>>> Stashed changes
            ...
            <View style={styles.wraprC}>
              <TouchableOpacity style={styles.buttons} onPress={toggleDropdown}>
                <Text style={{color: 'black'}}>
                  {selectedCat?.name || 'Sub Categories'}
                </Text>
                <Icons
...
<<<<<<< Updated upstream
            <TouchableOpacity style={styles.buttons} onPress={toggleDuration}>
              <Text style={{color: 'black'}}>
                {selectDur.duration + ' mints' || 'Select Duration'}
              </Text>
              <Icons
                name={isOpen ? 'chevron-up' : 'chevron-down'}
                size={24}
...
<<<<<<< Updated upstream
            <TouchableOpacity style={styles.buttons} onPress={toggleDurationT}>
              <Text style={{color: 'black'}}>
                {availability || 'Please add your availability'}
              </Text>
              <Icons
                name={isOpen ? 'chevron-up' : 'chevron-down'}
                size={24}
